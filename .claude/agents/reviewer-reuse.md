...
- **CPython + pip cache + requirements install** → `.github/actions/python-setup`. A raw
  `actions/setup-python` block, a separate `actions/cache` for `~/.cache/pip`, or a
  `python -m pip install -r requirements.txt` step in a leaf all duplicate it. Passing
  `dev-requirements: ""` is the supported way to skip test tooling — a second action or a
  conditional install step is not.
...
- **Image digest** → `java-docker.yml`'s `Resolve image reference` step and its
...
- **Test report publishing** → `dorny/test-reporter` with `if: always()` and
  `continue-on-error: true`. Only the `path`/`name` differ per language
  (`**/target/*-reports/TEST-*.xml` for Maven, `reports/TEST-*.xml` for pytest); both use
  `reporter: java-junit`, since pytest's `--junitxml` is JUnit XML. A new SARIF/JSON
  reporting path for the same reports is duplication.
- **Aggregate result gating** → the `build-gate` job in each master pipeline. A second
  "check the results" job duplicates it; add the phase to `build-gate`'s `needs:` **and**
  its `RESULTS` array instead. The two `build-gate` jobs are intentional twins — one per
  entry point — not duplication to collapse.
...
- **Pin bumps** → Dependabot's grouped weekly `github-actions` PR. A hand-bumped
  ...
  deliberate second commit that bumps the `java-setup`/`python-setup`/`ghcr-cleanup`
  self-references after editing a composite action).
...
gate that `java-release.yml` already applies.
...
- **Do not flag the deliberate per-workflow preamble.** Every leaf repeating
  `harden-runner` → `checkout` → `java-setup` → `chmod +x mvnw` (or `harden-runner` →
  `checkout` → `python-setup`) is a design decision: each workflow must be readable
  end-to-end and carry its own allowlist. Only flag duplication that is *not* that preamble.
- **Do not flag the Java and Python trees as duplicates of each other.** They are parallel
  by design — same shape, different toolchain. What *is* a finding: a Python leaf that
  re-implements something already shared (`tag.yml`, `deploy-gitops.yml`, `ghcr-cleanup`,
  the Trivy cache trio, the `image-digest` output contract) rather than calling it.
...
- Don't flag the `java-verify.yml` / `java-release.yml` grouping layers as pass-through
