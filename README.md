...
flowchart TD
    subgraph s1["① Build"]
        B["build<br/>compile · package · dep-graph"]
    end
    ...
    subgraph s2["② Verify — parallel"]
        ...
        U["unit-tests"]
        I["integration-tests"]
        SEC["security<br/>CodeQL · Gitleaks"]
    end
...
| `docker.yml` | Buildpack image → GHCR (`pr-<n>` / `main-<sha>`, keeps 3 newest); Trivy scan, SBOM, keyless Cosign signature |
...
- **Signature verification gate** — `verify-image` runs `cosign verify` on the pushed image before `deploy-gitops` promotes it, so an unsigned or tampered image blocks the deploy.
- **SBOM + vulnerability scan** — Syft CycloneDX SBOM and a Trivy `CRITICAL,HIGH` scan run against the built image.
